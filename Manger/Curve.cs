        ...
        private void bindDust()
        {
            ...
            try
            {
                using (MySqlConnection connPi = new MySqlConnection(serverConnectionString))
                {
                    ...
                    string sql = "SELECT top a.dust,a.date FROM `tb_project` b INNER JOIN `tb_monitordata01` a ON a.Project_ID=b.Project_ID WHERE b.Project_Name='" + projectName + "' ORDER BY a.date DESC ";
